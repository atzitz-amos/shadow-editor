...
export class SynSuiteWindowRenderer implements SynSuiteResultRenderer {
    ...
    render(results: Map<string, SynAutomatedTestResult>, focusTest?: string): void {
        ...
        popup.document.write(this.buildHtml(results, focusTest));
    ...
    private buildHtml(results: Map<string, SynAutomatedTestResult>, focusTest?: string): string {
        ...
        return `<!DOCTYPE html>
                ...
                <script>
                  ...
                  (function focusOnLoad() {
                    const focusKey = ${JSON.stringify(focusTest ?? null)};
                    if (!focusKey) return;
                
                    const card = document.querySelector('.test-card[data-key="' + CSS.escape(focusKey) + '"]');
                    if (!card) return;
    ...
    <div class="warnings">
      ${result.warnings.map(w => `<div class="warn-item">⚠ ${this.escape(w)}</div>`).join("")}
        ...
        const treeDiff = `<div class="section-label section">AST Tree</div>
    ...
    <div class="timing-grid">
      ...
      <div class="timing-cell" style="border-color:#2e3147">
        ...
        return `<div class="test-card ${pass ? "passed" : "failed"}" data-key="${this.escape(key)}">
