...
export abstract class IncrementalLexer implements ILexer {
    public relex(event: DocumentModificationEvent): TextRange {
        let cache = event.getDocument().getTokenCache();

        let startOffset = cache.findPreviousWhitespaceToken(event.getOffset());
        let endOffset = cache.findNextWhitespaceToken(event.getAffectedRange().end) + event.getInsertedText().length;
        let changedOffset = event.getTextDelta();
        ...
        return cache.update(startOffset, endOffset - changedOffset, tokens, changedOffset);
    }
