...
export class ApplyQuickFixAction extends AbstractAction {
    ...
    run(ctx: KeybindContext): void {
        ...
        if (quickFix) {
            const replacement = new SynModificationTree(editor.getCurrentLanguage()!, editor.getCodeAnalysisService().getSynFile().getSynDocument().getTree(), editor.getOpenedDocument().getTextContent());
